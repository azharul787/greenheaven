...
<div class="single-sec" id="decoration_product_details">
	 <div class="container">
		
		 <!-- start content -->	
		 <div><button class="btn btn-default btn-xm glyphicon glyphicon-chevron-left" onclick="back_decoration()"> Back</button></div>
		 <div class="col-md-12 det">
			 <?php
				...
				$id   = trim($_REQUEST['f_id']);
					...
					$query = $database->query("SELECT * FROM decoration_plants where id = $id ");
			...
			<div class="single_left">
				<img src="admin/decoration/<?php echo $row['image_name'] ;?>" height="300" width="300" />
			</div>
					...
					<div class="cost">
						 <div class="prdt-cost">
							 <ul>
								 ...
								 <li>
									<input type="button" class="item_add items btn  my-cart-btn" value="ADD TO CART" data-id="<?php echo $row['id'] ; ?>" data-name="<?php echo $row['name'] ; ?>" data-summary="<?php echo $row['description'] ; ?>" data-price="<?php echo $row['price'] ; ?>" data-quantity="1" data-image="admin/decoration/<?php echo $row['image_name'] ; ?>">
								</li>
							 </ul>
						 </div>
						 <div class="check">
			...
			<div class="arrivals">	
				...
				<?php 
				 include('database_connection.php');
				//$database = new mysqli('localhost','root','','green_heaven');
				$query = $database->query("SELECT * FROM decoration_plants ORDER BY id LIMIT 4");
				if($result = $query->num_rows > 0){
				while($row = $query->fetch_assoc())
					{	
				?>
				<div class="col-md-3 feature-grid jewel">
					 <a href="#" class="tablink" onclick="openEvent(event, 'decoration')" ><img src="admin/decoration/<?php echo $row['image_name']; ?>" alt=""/>	
