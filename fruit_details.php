...
<div class="single-sec" id="fruit_product_details">
	 <div class="container">
		
		 <!-- start content -->	
		 <div><button class="btn btn-default btn-xm glyphicon glyphicon-chevron-left" onclick="back_fruit()"> Back</button></div>
		 <div class="col-md-12 det">
			 <?php
				...
				$query = $database->query("SELECT * FROM fruit_plants where id = $id ");
			...
			?>
				 ...
				 <div class="single_left">
					<img src="admin/fruit/<?php echo $row['image_name'] ;?>" height="300" width="300" />
				 </div>
			...
			<div class="arrivals">	
				...
				<?php 
				 include('database_connection.php');
				//$database = new mysqli('localhost','root','','green_heaven');
				$query = $database->query("SELECT * FROM fruit_plants ORDER BY id LIMIT 4");
				if($result = $query->num_rows > 0){
				while($row = $query->fetch_assoc())
					{	
				?>
				<div class="col-md-3 feature-grid jewel">
					 <a href="#" class="tablink" onclick="openEvent(event, 'fruit')" ><img src="admin/fruit/<?php echo $row['image_name']; ?>" alt=""/>	
