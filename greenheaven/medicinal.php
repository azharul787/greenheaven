...
<div class="product-model" id="medicinal_product" >	 
	 <div class="container">
		 ...
		 <div class="col-md-12 product-model-sec" >
			<?php 
				...
				$query = $database->query("SELECT * FROM medicinal_plant ORDER BY id");
				if($result = $query->num_rows > 0){
				while($row = $query->fetch_assoc())
					{	
				?>
				
				<a href="#" onclick="medicinal_details( <?php echo $row['id']; ?>)">
					<div class="product-grid love-grid">
						<div class="product-img b-link-stripe b-animate-go  thickbox">
							<img src="admin/medicinal/<?php echo $row['image_name']; ?>" class="img-responsive" alt=""/>
							<div class="b-wrapper">
	...
	<script>
		function medicinal_details(f_id){
			var f_id = f_id
			$.ajax({
				type:"post",
				url:"medicinal_details.php",
				data:{f_id:f_id},
				success:function(result){
					$('#display_medicinal_details').html(result);
					$('#medicinal_product').hide();	
				}
			});
		}
	</script>
	
	<div id="display_medicinal_details">
