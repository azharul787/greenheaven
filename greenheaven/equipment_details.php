...
<div class="single-sec" id="equipment_product_details">
	 <div class="container">
		
		 <!-- start content -->	
		 <div><button class="btn btn-default btn-xm glyphicon glyphicon-chevron-left" onclick="back_equipment()"> Back</button></div>
		 <div class="col-md-12 det">
			 <?php
				...
				$query = $database->query("SELECT * FROM equipments where id = $id ");
			...
			<div class="single_left">
				<img src="admin/equipment/<?php echo $row['image_name'] ;?>" height="300" width="300" />
			</div>
						...
						<fieldset>					
							<section>
							     <div class="rating">
									...
									<label for="stars-rating-5"><i class="icon-star" data-toggle="modal" data-target="#myModal_feedback" ></i></label>
									<input type="radio" name="stars-rating" id="stars-rating-4" value="4">
									<label for="stars-rating-4"><i class="icon-star" data-toggle="modal" data-target="#myModal_feedback" ></i></label>
									<input type="radio" name="stars-rating" id="stars-rating-3" value="3">
									<label for="stars-rating-3"><i class="icon-star" data-toggle="modal" data-target="#myModal_feedback" ></i></label>
									<input type="radio" name="stars-rating" id="stars-rating-2" value="2 ">
									<label for="stars-rating-2"><i class="icon-star" data-toggle="modal" data-target="#myModal_feedback" ></i></label>
									<input type="radio" name="stars-rating" id="stars-rating-1" value="1" >
									<label for="stars-rating-1"><i class="icon-star" data-toggle="modal" data-target="#myModal_feedback"></i></label>
					...
					<div class="cost">
						 <div class="prdt-cost">
							 <ul>
								 ...
								 <li>
									<input type="button" class="item_add items btn  my-cart-btn" value="ADD TO CART" data-id="<?php echo $row['id'] ; ?>" data-name="<?php echo $row['name'] ; ?>" data-summary="<?php echo $row['description'] ; ?>" data-price="<?php echo $row['price'] ; ?>" data-quantity="1" data-image="admin/equipment/<?php echo $row['image_name'] ; ?>">
								</li>
							 </ul>
						 </div>
						 <div class="check">
			...
			<div class="arrivals">	
				...
				$query = $database->query("SELECT * FROM equipments ORDER BY id LIMIT 4");
				if($result = $query->num_rows > 0){
				while($row = $query->fetch_assoc())
					{	
				?>
				<div class="col-md-3 feature-grid jewel">
					 <a href="#" class="tablink" onclick="openEvent(event, 'equipment')" ><img src="admin/equipment/<?php echo $row['image_name']; ?>" alt=""/>	
			 ...
			 <div class="col-md-6 span_2_of_3">
				  <div class="contact-form">
					    <form action="#" method="post" id="coment_form_validation" name="coment_form">	
							<div>
						     	<span><label>Full name</label></span>
						    	<span><input name="fullname" type="text" id="fullname" class="fullname textbox" placeholder="Full Name"></span>
								<br><span class="error_message_color" id="fullname_error_message" style="color:red;"></span>
						    </div>
						    <div>
						    	<label>MESSAGE</label>
						   ...
						   <div>
						   		<span><input type="button" name="submit" onclick="coment_message()" class="mybutton" value="Submit"></span>
						  </div>
					    </form>
				    </div>
  				</div>
