...
<div class="single-sec" id="gardening_product_details">
	 <div class="container">
		
		 <!-- start content -->	
		 <div><button class="btn btn-default btn-xm glyphicon glyphicon-chevron-left" onclick="back_gardening()"> Back</button></div>
		 <div class="col-md-12 det">
			 <?php
				...
				$id   = trim($_REQUEST['f_id']);
					...
					$query = $database->query("SELECT * FROM gardening_plants where id = $id ");
			...
			<div class="single_left">
				<img src="admin/gardening/<?php echo $row['image_name'] ;?>" height="300" width="300" />
			</div>
			...
			<div class="arrivals">	
				...
				<?php 
				 include('database_connection.php');
				//$database = new mysqli('localhost','root','','green_heaven');
				$query = $database->query("SELECT * FROM gardening_plants ORDER BY id LIMIT 4");
				if($result = $query->num_rows > 0){
				while($row = $query->fetch_assoc())
					{	
				?>
				<div class="col-md-3 feature-grid jewel">
					 <a href="#" class="tablink" onclick="openEvent(event, 'gardening')" ><img src="admin/gardening/<?php echo $row['image_name']; ?>" alt=""/>	
