  ...
  var loadMyCartEvent = function(targetSelector){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        //'<a href="order_form.php" target="_blank"><button type="button" class="btn btn-primary ' + classCheckoutCart + '">Place Order</button></a>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function(){
      ...
      $.each(products, function(){
        ...
        $cartTable.append(
		  /* '<tr >' +
			  '<td class="text-center" style="width: 30px;">'Item Name'</td>' +
			  '<td>'Quantity'</td>' +
			  '<td >'Unit Price'</td>' +
			  '<td >'Quantity'</td>' +
          '</tr>'*/
